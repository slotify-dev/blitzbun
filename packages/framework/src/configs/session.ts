...
export default (envService: EnvContract): SessionOptions => {
  return {
    ...
    sameSite: envService.get('SESSION_SAME_SITE', 'Strict') as
      | 'Strict'
      | 'Lax'
      | 'None',
    domain: envService.get('SESSION_DOMAIN', undefined),
    csrfProtection:
      envService.get('SESSION_CSRF_PROTECTION', 'true') === 'true',
    regenerateOnAuth:
      envService.get('SESSION_REGENERATE_ON_AUTH', 'true') === 'true',
    rolling: envService.get('SESSION_ROLLING', 'false') === 'true',
    secret: envService.get('SESSION_SECRET', ''),
    strategy: envService.get('SESSION_STRATEGY', 'memory') as
      | 'redis'
      | 'memory',
  };
};
